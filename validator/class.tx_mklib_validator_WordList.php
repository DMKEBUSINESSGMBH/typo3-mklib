...
class tx_mklib_validator_WordList {
	 ...
	 * Prüft ob in einem String kein Wort, das blacklisted ist, vorkommt
  	...
  	public static function stringContainsNoBlacklistedWords ($word,$greedy = true, $sanitizeWord = true) {
    	if(empty($word))return true;//leer bedeutet kein Treffer

		$wordlistSrv = tx_mklib_util_ServiceRegistry::getWordlistService();
		$entry = $wordlistSrv->getBlacklistEntryByWord($word,$greedy,$sanitizeWord);
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/mklib/validator/class.tx_mklib_validator_WordList.php']) {
  include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/mklib/validator/class.tx_mklib_validator_WordList.php']);
}
